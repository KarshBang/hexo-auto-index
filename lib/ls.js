...
function createCallback(basePath) {
    const res = []
    let dir = {data: []}
    const replaceFix = remove_postfix ? '/' : '.html'
    function callback(filePath, filepathObj, isFile) {
        const reg = /^(\.(md|html))*$/
        if (reg.test(filepathObj.ext)) {
            dir.data.push({ path: path.relative(basePath, filePath.replace('.md', replaceFix)), name: filepathObj.name, type: 'file' })
        }
        if (!isFile) {
...
module.exports = function (basePath, diyDir, remove_postfix) {
    const [res, callback] = createCallback(basePath, remove_postfix)
